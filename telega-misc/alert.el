...

(defcustom alert-libnotify-command (executable-find "herbe")
...
(defvar alert-libnotify-command "herbe"
  "The command to use for libnotify notifications.")
...
(defun alert-libnotify-notify (info)
  ...
  (if alert-libnotify-command
      (let* ((title (plist-get info :title))
             ...
             (args (list alert-libnotify-command title message)))
        (apply #'call-process (car args) nil 0 nil (cdr args)))
    (alert-message-notify info)))
